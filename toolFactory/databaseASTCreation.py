...
pathFilenameStubFile = Path('/apps/astToolkit/typings/stdlib/ast.pyi')
...
if 3 == False: # pyright: ignore[reportUnnecessaryComparison]
...
def updateMatchArgs():
	# pathFilenameStubFile and pathFilenameDatabaseAST are correct
	# column 'ClassDefIdentifier' has `astClass.__name__`
	# make `listASTClasses` from that
	# don't change other columns
	# no missing values in the new columns
	for astClass in listASTClasses:
		Def=ast.ClassDef('j')
		c = astClass.__name__
		for node in ast.walk((astModule := ast.parse((source := pathFilenameStubFile.read_text())))):
			if isinstance(node, ast.ClassDef) and node.name==c:
				Def=node
				del node
				break
		...
		for node in ast.walk(Def):
			if isinstance(node, ast.Assign):
				print(c, ast.get_source_segment(source, node))
			# find every version of `__match_args__` for the class def
			# put the tuple of str in 'match_args'
			# put the correct Python minor version in 'match_argsVersionMinorMinimum'
