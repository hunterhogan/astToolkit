...
# There should be at least one test per comment
exec_tests = [
    # Module docstring
    "'module docstring'",
    # FunctionDef
...
# It should test all ast expressions
eval_tests = [
  # Constant(value=None)
  "None",
  # True
...
def main() -> None:
    if __name__ != '__main__':
        return
    if sys.argv[1:] == ['-g']:
        for statements, kind in ((exec_tests, "exec")
								, (single_tests, "single")
                                , (eval_tests, "eval")):
            print(kind+"_results = [")  # noqa: T201
            for statement in statements:
                tree = ast.parse(statement, "?", kind)
                print(f"{to_tuple(tree)!r},")  # noqa: T201
            print("]")  # noqa: T201
        print("main()")  # noqa: T201
        raise SystemExit

# ruff: noqa: E501
#### EVERYTHING BELOW IS GENERATED BY python tests/test_ast/snippets.py -g  #####
exec_results: list[tuple[Any, ...]] = [
